...
// Добавь это ВНУТРИ объекта Core:
ToggleChat() {
    ...
    // Переключаем класс (магия CSS сработает сама)
    panel.classList.toggle('collapsed');

    if (panel.classList.contains('collapsed')) {
        // Меняем иконку на "Развернуть"
        btn.innerHTML = '<i class="fa-solid fa-expand"></i>';
        if (wrapper) wrapper.style.display = 'none'; // Мгновенно скрываем контент
    } else {
        // Меняем иконку на "Свернуть"
        btn.innerHTML = '<i class="fa-solid fa-compress"></i>';
        if (wrapper) wrapper.style.display = 'block'; // Показываем контент обратно
